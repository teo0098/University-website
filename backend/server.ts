                ...
                if (result[0].semnumber < req.query.semester) {
                    ...
                    pool.query(select2, [`${req.query.major}`, req.query.semester], (err2, result2) => {
                        if (err2) {
                            ...
                            res.status(200).redirect(`/students/grades?data=${encodeURIComponent(result2[0])}
                            &data=${encodeURIComponent(result2[1])}&data=${encodeURIComponent(result2[2])}
                            &data=${encodeURIComponent(result2[3])}&data=${encodeURIComponent(result2[4])}
                            &data=${encodeURIComponent(result2[5])}&data=${encodeURIComponent(result2[6])}
                            &data=${encodeURIComponent(result2[7])}&data=${encodeURIComponent(result2[8])}`);
                        }
                    });
                }
            }
